...
const TryOnSidebar = () => (
  <div className="rounded-2xl h-full relative overflow-hidden" style={{ backgroundImage: `url(${bgShowroom})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
    ...
    <div className="relative z-10 p-5 flex flex-col h-full">
      <div className="flex items-center gap-2 mb-4"><Sparkles className="w-5 h-5 text-primary" /><h3 className="font-display text-subtitle font-bold text-foreground">Đặt Làm Theo Yêu Cầu</h3></div>
      ...
      <p className="font-body text-body text-muted-foreground mb-4 leading-relaxed font-light">Tạo nên chiếc vòng cổ duy nhất dành riêng cho bạn — từ chất liệu, kiểu dáng đến từng chi tiết nhỏ nhất.</p>
      <div className="space-y-3 mb-5 flex-1">
        {features.map((f, i) => (
          <div key={i} className="flex items-start gap-3 bg-card/70 rounded-xl p-3 transition-all duration-300 hover:bg-primary/15 hover:shadow-lg hover:scale-[1.03] cursor-default">
            ...
            <div><p className="font-body text-body font-semibold text-foreground">{f.title}</p><p className="font-body text-body text-muted-foreground font-light">{f.desc}</p></div>
          </div>
        ))}
      </div>
      <a href="/lien-he" className="btn-gold flex items-center justify-center gap-2 text-body px-6 py-3 rounded-full w-full font-semibold"><MessageCircle className="w-4 h-4" /> Liên Hệ Đặt Làm</a>
    </div>
  </div>
);
